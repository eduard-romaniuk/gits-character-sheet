(function (Sheet) {
  ...
  function orderItemsForDisplay(sectionKey, list) {
    ...
    list.filter((item) => !isAttachment(item)).forEach((weapon) => {
      out.push(weapon);
      attachments.forEach((attachment) => { if (attachment.extra.fittedId === weapon.instanceId) out.push(attachment); });
    });
  ...
  function renderLoadoutBar() {
    ...
    return '<div class="loadout-bar">'
      ...
      + (activeLoadout ? '<div class="loadout-manage">'
          + '<button class="icon text" data-action="loadoutEdit" data-loadout-id="' + activeLoadout.id + '">RENAME LOADOUT</button>'
          + (loadouts.length > 1 ? '<button class="icon delete" data-action="loadoutDelete" data-loadout-id="' + activeLoadout.id + '" aria-label="Delete loadout" title="Delete">' + Sheet.ICONS.delete + '</button>' : '')
          + '</div>' : '') + '</div>';
  }

  function renderSectionMarkup(sectionKey, label) {
    const isEquipmentSection = sectionKey === 'Equipment';
    const resolved = Sheet.resolveSectionItems(sectionKey);
    const list = isEquipmentSection ? resolved.filter((item) => item.extra.loadoutId === Sheet.state.activeLoadoutId) : resolved;
    let rows = '';
    orderItemsForDisplay(sectionKey, list).forEach((item) => {
      ...
      rows += '<div class="row-wrap' + (isAttachmentRow ? ' attachment' : '') + '">'
        + '<div class="row" draggable="true" data-section="' + sectionKey + '" data-id="' + item.instanceId + '">'
        + (isAttachmentRow ? '<div class="branch">└</div>' : '')
        + '<button class="chevron" data-action="toggleRow" data-section="' + sectionKey + '" data-id="' + item.instanceId + '"'
        + ' aria-expanded="' + open + '">' + (open ? '▾' : '▸') + '</button>'
        + '<button class="row-name" data-action="toggleRow" data-section="' + sectionKey + '" data-id="' + item.instanceId + '">' + escapeHtml(item.name) + '</button>'
        + (hasQty ? '<div class="quantity">'
            + '<button data-action="quantity" data-section="' + sectionKey + '" data-id="' + item.instanceId + '" data-delta="1" aria-label="restore">−</button>'
            + '<div class="count">' + (Math.max(0, totalQty - toNumber(extra.used, 0))) + ' / ' + totalQty + '</div>'
            + '<button data-action="quantity" data-section="' + sectionKey + '" data-id="' + item.instanceId + '" data-delta="-1" aria-label="use">+</button>'
            + '</div>' : '')
        + '<div class="spacer"></div>'
        + (meta ? '<div class="meta">' + escapeHtml(meta) + '</div>' : '')
        + '<button class="icon" data-action="edit" data-section="' + sectionKey + '" data-id="' + item.instanceId + '" aria-label="Edit" title="Edit">' + Sheet.ICONS.edit + '</button>'
        + '<button class="icon delete" data-action="del" data-section="' + sectionKey + '" data-id="' + item.instanceId + '" aria-label="Delete" title="Delete">' + Sheet.ICONS.delete + '</button>'
        + '</div>';
    ...
    return '<div class="panel" style="display:flex;flex-direction:column;gap:12px">'
      ...
      + '<button class="btn dashed" data-action="addFromLibrary" data-section="' + sectionKey + '">+ ADD</button>'
      + '</div>';
  }
